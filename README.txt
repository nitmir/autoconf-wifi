...
The builded binary import CA certificates (if needed) and install
a connection profile, so users just need to enter their credentials
and dont have to do the pain in the ass manual configuration.
...
Otherwise, the easiest way is to configure a wifi profile by hand using
windows interface and to export it with the following command :
...
A binary sample configuring the connection for the Cr@ns network
is avaible at http://ftp.crans.org/pub/logiciels/autoconf/Wifi.exe
